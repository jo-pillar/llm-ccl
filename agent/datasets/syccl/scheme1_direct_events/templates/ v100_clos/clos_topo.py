...
class ClosTopology(BaseTopology):
    ...
    def f_gpu2gpu(self):
        """Layer 0: DGX V100 host-local NVLink topology."""
        ...
        ring_channels = (
            ((0, 1, 2, 5, 6, 7, 4, 3), LinkSpec("25GBps", host_local_spec.link_spec.latency)),
            ((0, 2, 3, 1, 6, 4, 5, 7), LinkSpec("50GBps", host_local_spec.link_spec.latency)),
        )
        for host_id in range(host_local_spec.group_num):
            for ring, link_spec in ring_channels:
                assert len(ring) == gpu_per_host, "DGX V100 ring must cover all local GPUs."
                for index, gpu_i in enumerate(ring):
                    gpu_j = ring[(index + 1) % len(ring)]
                    ...
                    self.connect(src_node, dst_node, link_spec)
